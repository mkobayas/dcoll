...
/**
 * Distribute Hash Map Bucket<br>
 * バケット構造を第一キー(key)と第二キー(valueKey)で管理する。
 * 各バケットは第二キー(valueKey)のハッシュ値を元に分散バケットで管理される。<br>
 * 
 * @author mkobayas@redhat.com
 *
 */
@SuppressWarnings({ "rawtypes", "unchecked" })
abstract public class DHMBucket<K, VK, V> {
     ...
     * {@link DHMBucketTrasfer} が putされる。<br>
     * このTreeBucketTrasferは、バケット分割前のsubKeyでバケット分割後のデータを読み取るために用いられる。<br>
    ...
    public V put(K key, VK valueKey, V value) {
        ...
        DHMBucketMeta meta = (DHMBucketMeta) cache.get(key);

        if (meta == null) {
            // 新規キー
            meta = new DHMBucketMeta();
            meta.setBucketSize(0, 1);

            DHMBucketEntry bucket = new DHMBucketEntry();
        ...
        // バケット取得
        DHMBucketEntry<K, VK, V> bucket = (DHMBucketEntry) cache.get(subkey);
        
        if(bucket == null) {
            bucket = new DHMBucketEntry<>();
        }
        ...
        // バケット分割
        meta = new DHMBucketMeta(meta); // immutable
        meta.splitDown(index);
        int leftIndex = index;
        int rightIndex = index + 1;
        int splitPoint = meta.getHashRnge(rightIndex)[0];

        DHMBucketEntry newLeftBucket = new DHMBucketEntry();
        DHMBucketEntry newRightBucket = new DHMBucketEntry();
        ...
        // で実行することによりゴミ削除する想定。
        DHMBucketTrasfer tbt = new DHMBucketTrasfer();
     ...
     * {@link DHMBucketTrasfer} が putされる。<br>
     * このDHMBucketTrasferは、バケット統合前のsubKeyでバケット統合後のデータを読み取るために用いられる。<br>
     * <b>必ずkeyでクラスタワイドロックを取得すること。</b>
     * 
     * @param key 第一キー
     * @param valueKey 第二キー
    ...
    public V remove(K key, VK valueKey) {
        ...
        DHMBucketEntry<K, VK, V> anotherBucket = (DHMBucketEntry) cache.get(anotherSubKey);
        ...
        // new merged bucket
        DHMBucketEntry newBucket = new DHMBucketEntry();
    ...
    public Map<VK, V> getValues(K key) {
        ...
        if (meta != null) {
            ...
            for (SubKey subkey : subKeyList) {
                DHMBucketEntry bucket = getBucket(subkey);
                if (bucket != null) {
    ...
    public Map<VK, V> getValuesBySubKey(SubKey subKey) {
        ...
        DHMBucketEntry<K, VK, V> bucket = getBucket(subKey);
        if (bucket != null) {
    ...
    public boolean isContains(K key, VK valueKey) {
        ...
        DHMBucketEntry<K, VK, V> bucket = getBucket(meta.getSubKey(index));
        if (bucket != null && bucket.getValueMap().containsKey(valueKey)) {
            return true;
        }
    ...
    /**
     * TreeBucketTrasferを考慮して、指定されたバケットを取得する。
     * 
     * @param subKey バケットのサブキー
     * @return バケット
     */
    private DHMBucketEntry getBucket(SubKey subKey) {
        Object tmp = cache.get(subKey);
        if (tmp instanceof DHMBucketEntry) {
            return (DHMBucketEntry) tmp;
        } else if (tmp instanceof DHMBucketTrasfer) {
            DHMBucketEntry mergeBucket = new DHMBucketEntry();
            DHMBucketTrasfer trans = (DHMBucketTrasfer) tmp;
            for (SubKey transSubKey : trans.getTransSubKeyList()) {
                DHMBucketEntry actualBucket = getBucket(transSubKey);
    ...
    public static class DHMBucketMeta implements Serializable {
         ...
         * @param origin
         */
        public DHMBucketMeta(DHMBucketMeta origin) {
    ...
    public static class DHMBucketEntry<K, VK, V> implements Serializable {
        private static final long serialVersionUID = 1L;
    ...
    public static class DHMBucketTrasfer implements Serializable {
        private static final long serialVersionUID = 1L;
